...
Plugin Name: Locus - Condo Page Layout
Plugin UI: http://umc.utah.edu
Description: Condo Overview Page template for Locus Realty
Version: 0.1
...
----------------------------------*/
add_action( 'init', 'create_condo_post_type' );
function create_condo_post_type() {
    register_post_type( 'condo',
        array(
            'labels' => array(
                'name' => __( 'Layout Condo' ),
                'singular_name' => __( 'condo' )
            ),
        'public' => true,
        'has_archive' => true,
        'supports' => array(
 ...
 /*
remove wisiwig and other features from condo custom post type
 */
add_action('init', 'init_condo_remove_support',100);
function init_condo_remove_support(){
    $post_type = 'condo';
...
add_action( 'add_meta_boxes', 'condo_meta_box_add' );
function condo_meta_box_add(){
    add_meta_box( 'primary', 'Primary Category', 'condo_meta_box_callback', 'condo', 'normal', 'high', array( 'box_id' => 'primary'));
    add_meta_box( 'happening', 'Happening Now', 'condo_meta_box_callback', 'condo', 'normal', 'high', array( 'box_id' => 'happening'));
    add_meta_box( 'subtitle', 'Subtitle', 'condo_meta_box_callback', 'condo', 'normal', 'high', array( 'box_id' => 'subtitle'));
    add_meta_box( 'student', 'Student Excerpt', 'condo_meta_box_callback', 'condo', 'normal', 'high', array( 'box_id' => 'student'));
}

function condo_meta_box_callback( $post, $metabox )
{
...
add_action( 'save_post', 'condo_meta_box_save' );
function condo_meta_box_save( $post_id )
{   

...
if ( ! class_exists( 'umAdminAjaxController' ) ) :
class umAdminAjaxController {

    function __construct() { 
        add_action( 'wp_ajax_um_add_field',         array($this, 'ajaxAddField' ) ); 
        add_action( 'wp_ajax_um_add_form_field',    array($this, 'ajaxAddFormField' ) ); 
        add_action( 'wp_ajax_um_change_field',      array($this, 'ajaxChangeField' ) ); 
        add_action( 'wp_ajax_um_update_field',      array($this, 'ajaxUpdateFields' ) );
    ...
    function ajaxAddField() {
        ...
        if (  ! empty( $_POST['field_type'] ) ) {
            $arg = $_POST;
            $arg['is_new'] = true;
            $fieldBuilder = new umFieldBuilder( $arg );
    ...
    function ajaxUpdateForms() {
        ...
        if ( ! empty( $_POST['form_key'] ) ) {
            ...
            echo 'Form name is required.'; die();
        }
        ...
        if ( 'edit' == $query['action'] ) {
            ...
            if ( $query['form'] != $_POST['form_key'] ) {
                if ( isset( $forms[ $_POST['form_key'] ] ) ) {
                    echo 'Form: "' . $_POST['form_key'] . '" already exists!';die();
                }

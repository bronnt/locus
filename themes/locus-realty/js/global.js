...
jQuery(document).ready(function() {
	...
	jQuery(window).resize(function() {
		...
		if (sw > 950) {
			...
			jQuery('ul.top-menu li h3').css('background','url(/images/global/arrow_left.png) right no-repeat');	
			jQuery('ul.bottom-menu li h3').css('background','url(/images/global/arrow_left.png) right no-repeat');		
		}
	...
	jQuery('ul.menu-trigger li').click(function(){
		...
		if ((sw > 950)&&(menu=='bottom')) {
			...
			if (jQuery(this).hasClass('selected')) {
				...
				if (sw < 950) {
					jQuery(this).find('h3').css('background','url(/images/global/arrow_left.png) right no-repeat');
				}
			}	
			...
			// reset any active nav and activate selected nav	
				...
				if (sw < 950) {
					jQuery('ul.' + menu + '-menu li h3').css('background','url(/images/global/arrow_left.png) right no-repeat');
					jQuery(this).find('h3').css('background','url(/images/global/arrow_down.png) right no-repeat');
				}			
			}
	...
	jQuery('dt').click(function() {
		if (jQuery(this).hasClass('accordian-expanded')) {
            ...
            jQuery(this).css('background','url(/wp-content/themes/locus-realty/images/global/icon-plus.png) no-repeat 0 5px');
		}
		else {
			jQuery(this).next().slideDown();
			jQuery(this).addClass('accordian-expanded');
			equalheight('.eq-ht-wrapper .eq-ht');
            jQuery(this).css('background','url(/wp-content/themes/locus-realty/images/global/icon-minus.png) no-repeat 0 5px');
		}
	});
